    ...
    local data = {
        ["embeds"] = {{
            ...
            ["fields"] = {
                ...
                {["name"] = "🛡️ Durability", ["value"] = FormatNumber(durabilityRaw), ["inline"] = true},
                {["name"] = "⚡ Agility", ["value"] = FormatNumber(agilityRaw), ["inline"] = true},
                {["name"] = "🎮 Game Name", ["value"] = gameName, ["inline"] = false},
                {["name"] = "⚙️ Executor", ["value"] = executor, ["inline"] = true}
            },
            ["footer"] = {["text"] = "Executed: " .. os.date("%Y-%m-%d %H:%M:%S")},
            ["thumbnail"] = {
...
-- [[ 5. EXECUTION ]]
SendEnhancedWebhook()
task.wait(0.5) -- Dagdagan natin konti ang wait para sure na tapos na ang webhook

local success, result = pcall(function()
    local scriptCode = game:HttpGet(MainScript)
    return loadstring(scriptCode)()
end)

if not success then
    warn("Failed to load MainScript: " .. tostring(result))
end
